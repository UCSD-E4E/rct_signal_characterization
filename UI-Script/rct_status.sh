...
do
    ...
    # Check for SDR (return 1 for valid/0 for invalid)
    SDR_CHECK=`sudo uhd_find_devices --args="type=b200"`
    if [ "$SDR_CHECK" -eq 1 ]
    then
    ...
    if [ "$OVERALL_STATUS" -eq "1" ]
        ...
        $INSTALL_DIR/bin/rct_run "$OVERALL_STATUS"
